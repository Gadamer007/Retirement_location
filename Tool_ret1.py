...
# Normalize variables (0-1 range)
def normalize(df, column):
    df[column] = (df[column] - df[column].min()) / (df[column].max() - df[column].min())
    return df

data = normalize(data, "Cost of Living")

# Sidebar Filters
st.sidebar.subheader("Select Variables for Retirement Suitability")
selected_vars = []
sliders = {}
variables = list(column_mapping.values())[:-1]  # Exclude Cost of Living from selection
...
if selected_vars:
    df_selected = data[['Country', 'Cost of Living', 'Continent'] + selected_vars].dropna()

    for var in selected_vars:
        max_category = sliders[var]  
        df_selected = df_selected[df_selected[var] <= max_category]

    df_selected['Retirement Suitability'] = df_selected[selected_vars].mean(axis=1)

    # Rank-normalize Retirement Suitability (0-1 range)
    df_selected = normalize(df_selected, "Retirement Suitability")

    # Scatter Plot
    fig_scatter = px.scatter(
        df_selected, x="Retirement Suitability", y="Cost of Living", text="Country", color=df_selected['Continent'],
        title="Retirement Suitability vs Cost of Living", labels={"Cost of Living": "Cost of Living (0 - 1)", "Retirement Suitability": "Retirement Suitability (0 - 1)"},
